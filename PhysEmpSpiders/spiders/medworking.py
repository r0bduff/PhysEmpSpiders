...
class MedworkingSpider(scrapy.Spider):
    name = 'medworking'
    ...
    def parse(self, response):
        for post in response.css('#JobList a'):
            try:
...
#Parse listing page
    def parse_listing(self, response):
        ...
        if(yn == 'Yes'):
            ...
            bus_name = str(response.css('#JobDetail:nth-child(1) tr:nth-child(1) .data::text').get()).replace('\r','').replace('\t','').replace('\n','').strip()
        ...
        try:
            job = Item({
                ...
                'source_site': 'medworking',
                'url': response.meta['url'],
                'description': str(response.css('.job-right-panel::text').extract()).replace('\r','').replace('\t','').replace('\n',''),
